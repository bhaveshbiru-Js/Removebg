...
img = ImageOps.exif_transpose(img).convert("RGB")# read image
if img.width > 1500:
...
filename = filename()
out.save(filename)
print(f"file has been saved in : {filename}")import warnings
warnings.filterwarnings("ignore")
import numpy as np
from PIL import Image , ImageOps
import time
...
remover = Remover(mode='fast') # default setting
# remover = Remover(mode='fast', jit=True, device='cuda:0', ckpt='~/latest.pth') # custom setting
# remover = Remover(mode='base-nightly') # nightly release checkpoint
parse  = argparse.ArgumentParser()
parse.add_argument("image",help="input image",type=str)
...
filename = filename()
out.save(filename)
print(f"https://eventtechsolutions.in/tool/background/{filename}")